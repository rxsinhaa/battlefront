...
export function BentoAccordion() {
  ...
  return (
    <section id="features" className="w-full py-16 px-4 bg-oceanic-noir border-t border-nocturnal-expedition">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-10">
          <h2 className="text-3xl md:text-5xl font-bold font-mono text-arctic-powder mb-3">
            System Operations
          </h2>
          <p className="text-mystic-mint text-sm md:text-base max-w-2xl mx-auto font-sans">
        ...
        {/* Grid View: Bento Layout (md: and above viewports) */}
        <div className="hidden md:grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 auto-rows-fr">
          
          {/* Card 1: Neural Compute Engine (col-span-2, row-span-2) */}
          <article
            id="desktop-bento-card-0"
            onMouseEnter={() => setActiveIndex(0)}
            className={`p-6 rounded-2xl border transition-all duration-200 ease-out flex flex-col justify-between min-h-[285px] md:col-span-2 lg:col-span-2 lg:row-span-2 cursor-pointer relative z-10 overflow-visible ${
              activeIndex === 0
          ...
          >
            ...
            <div 
              className={`absolute -inset-8 bg-forsythia/10 blur-3xl rounded-full transition-opacity duration-300 pointer-events-none -z-10 ${
                activeIndex === 0 ? 'opacity-100' : 'opacity-0'
              }`}
            />

            <div className="flex justify-between items-start">
              <div className="p-2.5 bg-neutral-900/40 rounded-xl border border-nocturnal-expedition/30">
                <img src={ITEMS[0].icon} alt="" width={20} height={20} className="text-forsythia" />
              </div>
            ...
            <div className="my-4 flex-grow flex items-center justify-center">
              <svg className="w-full max-w-[240px] h-28 text-arctic-powder/15" viewBox="0 0 200 100" fill="none">
                ...
                <circle cx="40" cy="50" r="5" className={`fill-oceanic-noir stroke-current transition-colors duration-200 ${activeIndex === 0 ? 'text-forsythia animate-pulse' : 'text-mystic-mint/40'}`} strokeWidth="1.5" />
                <circle cx="100" cy="20" r="5" className={`fill-oceanic-noir stroke-current transition-colors duration-200 ${activeIndex === 0 ? 'text-deep-saffron' : 'text-mystic-mint/40'}`} strokeWidth="1.5" />
                <circle cx="100" cy="80" r="5" className={`fill-oceanic-noir stroke-current transition-colors duration-200 ${activeIndex === 0 ? 'text-deep-saffron' : 'text-mystic-mint/40'}`} strokeWidth="1.5" />
                <circle cx="160" cy="50" r="5" className={`fill-oceanic-noir stroke-current transition-colors duration-200 ${activeIndex === 0 ? 'text-forsythia animate-pulse' : 'text-mystic-mint/40'}`} strokeWidth="1.5" />
              </svg>
            </div>

            <div>
              <h3 className="text-2xl font-mono font-bold text-arctic-powder mb-2">{ITEMS[0].title}</h3>
              <p className="text-mystic-mint text-xs leading-relaxed max-w-lg font-sans">{ITEMS[0].description}</p>
            </div>
          </article>

          {/* Card 2: Autonomous Vector Pipelines (md:col-span-1, lg:col-span-1) */}
          <article
            id="desktop-bento-card-1"
            onMouseEnter={() => setActiveIndex(1)}
            className={`p-6 rounded-2xl border transition-all duration-200 ease-out flex flex-col justify-between min-h-[285px] md:col-span-1 lg:col-span-1 cursor-pointer relative z-10 overflow-visible ${
              activeIndex === 1
                ? 'bg-nocturnal-expedition border-forsythia shadow-lg shadow-forsythia/10 scale-[1.01]'
                : 'bg-neutral-900/50 border-nocturnal-expedition/30 hover:border-nocturnal-expedition/80'
            }`}
          >
            ...
            <div className="flex justify-between items-start">
              <div className="p-2.5 bg-neutral-900/40 rounded-xl border border-nocturnal-expedition/30">
                <img src={ITEMS[1].icon} alt="" width={20} height={20} />
              </div>
              <span className="text-xs font-mono uppercase tracking-widest text-mystic-mint/40">Core 02</span>
            </div>

            <div className="my-4 flex-grow flex items-center justify-center">
              <svg className="w-full max-w-[140px] h-24 text-arctic-powder/15" viewBox="0 0 200 100" fill="none">
                ...
                {activeIndex === 1 ? (
                  <>
                    <circle r="4" className="fill-forsythia">
                      <animateMotion dur="2.5s" repeatCount="indefinite" path="M20,50 L70,50 C90,50 110,20 130,20 L180,20" />
                    </circle>
                    <circle r="4" className="fill-deep-saffron">
                      <animateMotion dur="3.5s" repeatCount="indefinite" path="M70,50 C90,50 110,80 130,80 L180,80" />
                    </circle>
                  </>
                ) : (
                  <>
                    <circle cx="90" cy="35" r="3.5" className="fill-mystic-mint/30" />
                    <circle cx="120" cy="80" r="3.5" className="fill-mystic-mint/30" />
                  </>
                )}
              </svg>
            </div>
            ...
            <div>
              <h3 className="text-xl font-mono font-bold text-arctic-powder mb-2">{ITEMS[1].title}</h3>
              <p className="text-mystic-mint text-xs leading-relaxed font-sans">{ITEMS[1].description}</p>
            </div>
          </article>

          {/* Card 3: Telemetry Orchestration (md:col-span-1, lg:col-span-1) */}
          <article
          ...
          >
            ...
            <div className="flex justify-between items-start">
              <div className="p-2.5 bg-neutral-900/40 rounded-xl border border-nocturnal-expedition/30">
                <img src={ITEMS[2].icon} alt="" width={20} height={20} />
              </div>
              <span className="text-xs font-mono uppercase tracking-widest text-mystic-mint/40">Core 03</span>
            </div>

            <div className="my-4 flex-grow flex items-center justify-center">
              <svg className="w-full max-w-[140px] h-24 text-arctic-powder/15" viewBox="0 0 200 100" fill="none">
                <circle cx="100" cy="50" r="35" stroke="currentColor" strokeWidth="1" strokeDasharray="4 4" />
                <circle cx="100" cy="50" r="20" stroke="currentColor" strokeWidth="1" />
                <circle cx="100" cy="50" r="8" stroke="currentColor" strokeWidth="1" />
                <line x1="100" y1="15" x2="100" y2="85" stroke="currentColor" strokeWidth="1" strokeDasharray="2 2" />
                <line x1="65" y1="50" x2="135" y2="50" stroke="currentColor" strokeWidth="1" strokeDasharray="2 2" />
                <line
                  x1="100"
                  y1="50"
                  x2="130"
            ...
            <div>
              <h3 className="text-xl font-mono font-bold text-arctic-powder mb-2">{ITEMS[2].title}</h3>
              <p className="text-mystic-mint text-xs leading-relaxed font-sans">{ITEMS[2].description}</p>
            </div>
          </article>
        </div>

        {/* Mobile View: Fluid Accordion (below md: viewports) */}
              ...
              >
                ...
                >
                  ...
                  <div className={`transition-transform duration-200 ${isOpen ? 'rotate-180' : 'rotate-0'}`}>
                    <img src="/chevron-down.svg" alt="" width={18} height={18} />
                  </div>
                </button>
                ...
                >
                  <div className="overflow-hidden">
                    <div className="px-6 pb-6 pt-1">
                      <p className="text-mystic-mint text-sm mb-4 leading-relaxed font-sans">
                        {item.description}
                      </p>
